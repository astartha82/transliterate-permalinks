...
class TP_Plugin {
	 ...
	 *
	 * @param $post_types
	 * 		Array of post types to transliterate. Defaults to array('post', 'page').
	 * @param $taxonomies
	 * 		Array of taxonomies to transliterate. Defaults to array('category', 'post_tag').
	 *
	 */ 		
	function transliterate_all() {
		...
		if ($post_types != false) {
			...
			foreach ($posts as $p) {
				$trans = $this->transliterate_permalinks(urldecode($p->post_name));
				wp_update_post(
					array(
						'ID' => $p->ID,
						'post_name' => $trans
		...
		if ($taxonomies != false) {
			...
			foreach ($taxes as $tax) {
				$trans = $this->transliterate_permalinks(urldecode($tax->slug));
				wp_update_term($tax->term_id, $tax->taxonomy, array('slug' => $trans));
			}
		}
		echo 'All done!';
		wp_die();
	...
	function transliterate_permalinks($title) {
		return $this->transliterate($title);
	}

	function transliterate($string, $unknown = '?', $source_langcode = NULL) {
		...
		$result = str_replace('ou', 'u', $result);
		return $result;
	}
